...
mod tests {
    ...
    #[tokio::test]
    async fn create_get_cover_art_url() -> anyhow::Result<()> {
        let auth = AuthBuilder::new("peter", "v0.16.1")
            .salt_for_test("")
            .hashed("change_me_password");
        let client = Client::new("https://target.com", auth);
        let url = client.get_cover_art_url("testId", None)?;

        check_eq!(
            "https://target.com/rest/getCoverArt?u=peter&v=v0.16.1&c=submarine-lib&t=d4a5b2db9781fba37ec95f0312ade67a&s=&f=json&id=testId",
            &url.to_string()
        );
        Ok(())
    }
