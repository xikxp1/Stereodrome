...
mod tests {
    ...
    #[test]
    fn conversion_get_lyrics_empty() -> anyhow::Result<()> {
        let response_body = r#"
            {
              "subsonic-response": {
                "status": "ok",
        ...
        if let ResponseType::Lyrics { lyrics } = response.data {
            check_eq!(lyrics.value.as_deref(), Some(""));
        } else {
    ...
    #[test]
    fn conversion_get_lyrics() -> anyhow::Result<()> {
        let response_body = r#"
            {
                "subsonic-response": {
                    "status": "ok",
        ...
        println!("{response:?}");
        if let ResponseType::Lyrics { lyrics } = response.data {
            check_eq!(lyrics.artist.as_deref(), Some("Ella Fitzgerald"));
            check_eq!(
