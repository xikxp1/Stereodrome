...
use crate::operations::settings::{SyncSettings, manual_offline_enabled, read_sync_settings};
...
pub fn start_library_sync_scheduler(
    ...
    thread::spawn(move || {
        ...
        while running.load(Ordering::Acquire) {
            ...
            let settings = read_sync_settings(&state.settings);
            let due_job = match next_due_sync_job(&state, &settings) {
                Ok(job) => job,
                Err(e) => {
...
pub async fn sync_library(state: &DesktopState) -> AppResult<SyncResult> {
    if manual_offline_enabled(&state.settings) {
        return Err(AppError::OfflineMode);
    }

    run_sync_job_with_status(state, SyncJobKind::Incremental, SyncTrigger::Manual).await
}

pub async fn reconcile_library_state(state: &DesktopState) -> AppResult<SyncResult> {
    if manual_offline_enabled(&state.settings) {
        return Err(AppError::OfflineMode);
    }

    run_sync_job_with_status(state, SyncJobKind::FullReconcile, SyncTrigger::Manual).await
}

pub fn get_library_sync_status(state: &DesktopState) -> AppResult<LibrarySyncStatus> {
    let settings = read_sync_settings(&state.settings);
    read_library_sync_status(state, &settings)
}
...
async fn run_sync_job_with_status(
    ...
    emit_library_sync_status_changed(state);
    ...
    if trigger == SyncTrigger::Scheduled {
        emit_library_content_updated(state, new_item_counts.into_event(job));
    }

    Ok(result)
}

async fn run_incremental_sync(state: &DesktopState) -> AppResult<(SyncResult, NewItemCounts)> {
...
async fn run_full_reconcile_sync(state: &DesktopState) -> AppResult<(SyncResult, NewItemCounts)> {
    if !state.client.is_connected() {
        return Err(AppError::NotConnected);
    }
...
fn emit_library_sync_status_changed(state: &DesktopState) {
    let settings = read_sync_settings(&state.settings);
    match read_library_sync_status(state, &settings) {
        Ok(status) => state.events.library_sync_status_changed(status),
        Err(e) => warn!("Failed to emit library sync status event: {e}"),
    }
}

fn emit_library_content_updated(state: &DesktopState, event: LibraryContentUpdatedEvent) {
    if event.has_new_items {
        state.events.library_content_updated(event);
    }
}
...
fn rebuild_search_index_from_db(state: &DesktopState) -> AppResult<()> {
...
pub fn get_artists(state: &DesktopState) -> AppResult<Vec<Artist>> {
    let db = state.db.lock_recover();
...
pub fn get_album_count(state: &DesktopState) -> AppResult<i64> {
    let db = state.db.lock_recover();
    let count: i64 = db.query_row("SELECT COUNT(*) FROM albums", [], |row| row.get(0))?;
    Ok(count)
}

pub fn get_albums(state: &DesktopState, artist_id: Option<String>) -> AppResult<Vec<Album>> {
    let db = state.db.lock_recover();
...
pub async fn get_album_list(
    ...
    let order = crate::client::AlbumListOrder::parse(&list_type)
...
pub async fn get_scan_status(state: &DesktopState) -> AppResult<ScanStatus> {
    if manual_offline_enabled(&state.settings) {
        return Err(AppError::OfflineMode);
    }
...
pub async fn start_scan(state: &DesktopState) -> AppResult<ScanStatus> {
    if manual_offline_enabled(&state.settings) {
        return Err(AppError::OfflineMode);
    }
