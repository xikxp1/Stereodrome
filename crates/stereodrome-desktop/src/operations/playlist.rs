...
use crate::operations::cover_art::preserve_cover_art_for_offline;
...
pub async fn sync_playlists(state: Arc<DesktopState>) -> AppResult<i32> {
    ...
    let _ = reconcile_saved_playlists_offline_inner(Arc::clone(&state)).await?;
    remove_unprotected_cached_songs(Arc::clone(&state), old_saved_song_ids)?;
...
pub fn get_playlists(state: &DesktopState) -> AppResult<Vec<Playlist>> {
    let db = state.db.lock_recover();
...
pub async fn delete_playlist(state: Arc<DesktopState>, playlist_id: String) -> AppResult<()> {
    if manual_offline_enabled(&state.settings) {
        return Err(AppError::OfflineMode);
    }
    ...
    remove_unprotected_cached_songs(Arc::clone(&state), removed_song_ids)?;
...
pub async fn add_songs_to_playlist(
    ...
    if playlist_saved_offline(&state, &playlist_id)? {
        let _ = download_playlist_to_cache(Arc::clone(&state), &playlist_id).await?;
    }
...
pub async fn set_playlist_saved_offline(
    ...
    if saved_offline {
        ...
        match download_playlist_to_cache(Arc::clone(&state), &playlist_id).await {
        ...
        let (removed_count, skipped_protected_count) =
            remove_unprotected_cached_songs(Arc::clone(&state), song_ids)?;
...
pub async fn reconcile_saved_playlists_offline(
    ...
    reconcile_saved_playlists_offline_inner(state).await
}

/// Re-fetch a single playlist from server and update local cache
async fn refresh_playlist_cache(state: &DesktopState, playlist_id: &str) -> AppResult<()> {
...
fn playlist_song_ids(state: &DesktopState, playlist_id: &str) -> AppResult<HashSet<String>> {
    let db = state.db.lock_recover();
...
fn playlist_saved_offline(state: &DesktopState, playlist_id: &str) -> AppResult<bool> {
    Ok(playlist_offline_saved_at(state, playlist_id)?.is_some())
}

fn playlist_offline_saved_at(state: &DesktopState, playlist_id: &str) -> AppResult<Option<String>> {
    let db = state.db.lock_recover();
    Ok(db
        .query_row(
...
async fn download_playlist_to_cache(state: Arc<DesktopState>, playlist_id: &str) -> AppResult<i32> {
    ...
    for cover_art_id in cover_art_ids {
        if let Err(error) =
            preserve_cover_art_for_offline(&state, &state.client, &cover_art_id).await
        {
            warn!(
