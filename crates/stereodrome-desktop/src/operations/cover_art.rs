...
pub const PRESERVED_COVER_ART_SIZE: i32 = 800;
const FALLBACK_COVER_ART_SIZES: [i32; 5] = [PRESERVED_COVER_ART_SIZE, 200, 128, 96, 64];

/// Get the cover art cache directory path
fn get_cache_dir(state: &DesktopState) -> AppResult<PathBuf> {
    cover_cache_dir(&state.paths, &state.settings)
}
...
pub async fn preserve_cover_art_for_offline(
    state: &DesktopState,
    client: &SubsonicClientHandle,
    cover_art_id: &str,
) -> AppResult<()> {
...
pub async fn get_song_cover_art(
    ...
    match cover_art_id {
        Some(id) if !id.is_empty() => {
            let data_url = get_cover_art(state, id, size).await?;
            Ok(Some(data_url))
        }
        _ => Ok(None),
    }
